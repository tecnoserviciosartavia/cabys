    ...
    def _update_catalog_from_excel_file(self):
        ...
        if self.cabys_excel_file:
            ...
            for row in rows:
                ...
                tax = 0.0 if row[products_map['tax']].value in ('Exento', 'na') else float_value(row[products_map['tax']].value[:-1]) 
    ...
    def _analyze_excel_file(self):
        ...
        if self.cabys_excel_file:
            ...
            for row in rows:
                ...
                impuesto = 0.0 if row[products_map['tax']].value in ('Exento', 'na') else float_value(row[products_map['tax']].value[:-1]) 
